 ...
 public:
  ...
  void Initialize() override
  {
    ...
    if (config_.system_clock == SystemClockSelect::kPll)
    {
      ...
      {
        // 0 Wait states
        flash->ACR = sjsu::bit::Insert(
            flash->ACR, 0b000, sjsu::bit::MaskFromRange(0, 2));
      }
      else if (24_MHz <= pll_clock_rate_ && pll_clock_rate_ <= 48_MHz)
      {
        // 1 Wait state
        flash->ACR = sjsu::bit::Insert(
            flash->ACR, 0b001, sjsu::bit::MaskFromRange(0, 2));
      }
      else
      {
        // 2 Wait states
        flash->ACR = sjsu::bit::Insert(
            flash->ACR, 0b010, sjsu::bit::MaskFromRange(0, 2));
      }
    }
  ...
  units::frequency::hertz_t GetClockRate(ResourceID id) const override
  {
    switch (id.device_id)
    {
      case kI2s.device_id: return pll_clock_rate_;
      case kUsb.device_id: return usb_clock_rate_;
      case kFlitf.device_id: return kHighSpeedInternal;
      ...
      case kSystemTimer.device_id: [[fallthrough]];
      case kCpu.device_id: return ahb_clock_rate_;

      // APB1 Timers
      case kTimer2.device_id: [[fallthrough]];
      case kTimer3.device_id: [[fallthrough]];
      case kTimer4.device_id: [[fallthrough]];
      case kTimer5.device_id: [[fallthrough]];
      case kTimer6.device_id: [[fallthrough]];
      case kTimer7.device_id: [[fallthrough]];
      case kTimer12.device_id: [[fallthrough]];
      case kTimer13.device_id: [[fallthrough]];
      case kTimer14.device_id: return timer_apb1_clock_rate_;

      // APB2 Timers
      case kTimer1.device_id: [[fallthrough]];
      case kTimer8.device_id: [[fallthrough]];
      case kTimer9.device_id: [[fallthrough]];
      case kTimer10.device_id: [[fallthrough]];
      case kTimer11.device_id: return timer_apb2_clock_rate_;

      case kAdc1.device_id: [[fallthrough]];
      case kAdc2.device_id: [[fallthrough]];
      case kAdc3.device_id: return adc_clock_rate_;
    }

    if (id.device_id < kAPB1)
    {
      return ahb_clock_rate_;
    }

    if (kAPB1 <= id.device_id && id.device_id < kAPB2)
    {
      return apb1_clock_rate_;
    }

    if (kAPB2 <= id.device_id && id.device_id < kBeyond)
    {
